    ...
    <div class="flex flex-row w-full">
        ...
        <div class="w-1/3 h-full bg-gray-500">
            <details class="rounded-md shadow-md p-4">
                <summary class="cursor-pointer bg-gray-200 hover:bg-gray-300 py-2 px-4 rounded-md outline-none ">
                    Tipo de Falta
                </summary>
                <div class="flex flex-col justify-center items-center mt-2">
                    <!-- Using utilities: -->
                    <button class="w-1/2 bg-blue-500 hover:bg-blue-700 text-white font-bold my-2 py-2 px-4 rounded">
                        Falta
                    </button>
                    <!-- Using utilities: -->
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const drawRoute = () => {
            ...
            const markerFinal = L.marker(route[route.length - 1]).addTo(map).bindPopup('I am a marker.');
        }
    },
    mounted() {
        this.fetch_type_fol();
    ...
    methods: {
        async fetch_type_fol() {
            axios.get('type_foul')
                .then(response => {
                    this.type_fol = response.data
                    console.log(this.type_fol)
                })
                .catch(error => {
                    console.log(error)
                })
